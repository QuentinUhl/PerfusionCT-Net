...
def train(arguments):
    ...
    # If the model uses additional medical data
    if train_dataset.get_use_cd():
        for epoch in range(model.which_epoch, train_opts.n_epochs):
            ...
            # Training Iterations
            for epoch_iter, (images, labels, clinical_data, indices) in tqdm(enumerate(train_loader, 1), total=len(train_loader)):
                # Make a training update
                model.set_input(images, labels, clinical_data)
            ...
            for epoch_iter, (images, labels, clinical_data, indices) in tqdm(enumerate(valid_loader, 1), total=len(valid_loader)):
    ...
    # If the model doesn't use additional medical data
    else:
        for epoch in range(model.which_epoch, train_opts.n_epochs):
            print('(epoch: %d, total # iters: %d)' % (epoch, len(train_loader)))
            train_volumes = []
